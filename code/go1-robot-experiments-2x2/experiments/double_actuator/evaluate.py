...
def validate_training(
  ...
  checks = {
      ...
      "checkpoint": run.get("terminal_checkpoint") == f"checkpoints/{ACTUAL_STEPS:012d}",
      "training_experiment_commit": (
          run.get("experiment_commit") == training_commit
          == result.get("experiment_commit")
      ),
      "core_commit": run.get("core_commit") == identity["core_commit"],
      "pair_manifest": run.get("pair_manifest_sha256") == PAIR_MANIFEST_SHA256,
...
def evaluate(
  ...
  evidence = validate_training(
      training, model, seed, training_commit, identity,
  )
  output = training / "heldout" / f"shard-{shard}"
  if output.exists():
  ...
  try:
    ...
    record = {
        ...
        **identity,
        ...
        "training_result_status": evidence["result"]["status"],
...
def main() -> None:
  ...
  evaluate(
      Path(args.training_root).resolve(), args.model, args.seed, args.shard,
      args.training_commit, args.require_gpu,
  )
